...
S1apIfaceInfo::S1apIfaceInfo (Address remoteIpAddr, Ptr<Socket> localCtrlPlaneSocket)
{
...
EpcS1apEnb::EpcS1apEnb (Ptr<Socket> localSocket, PacketSocketAddress epcIabApplicationAddress, uint16_t cellId, uint16_t mmeId)
  : m_s1apUdpPort (36412) // As defined by IANA
{
  NS_LOG_FUNCTION (this);
  AddS1apInterface(cellId, mmeId, localSocket, epcIabApplicationAddress);
  m_s1apSapProvider = new MemberEpcS1apSapEnbProvider<EpcS1apEnb> (this);
}
                       ...
                       Ptr<Socket> localS1apSocket)
{
  ...
  m_s1apInterfaceSockets [mmeId] = Create<S1apIfaceInfo> (InetSocketAddress (mmeAddress, m_s1apUdpPort), localS1apSocket); 
...
void
EpcS1apEnb::AddS1apInterface (uint16_t enbId, uint16_t mmeId, Ptr<Socket> localS1apSocket, PacketSocketAddress epcIabApplicationAddress)
{
  NS_LOG_FUNCTION (this << enbId << mmeId << localS1apSocket);

  localS1apSocket->SetRecvCallback (MakeCallback (&EpcS1apEnb::RecvFromS1apSocket, this));

  NS_ASSERT_MSG (m_s1apInterfaceSockets.find (mmeId) == m_s1apInterfaceSockets.end (),
                 "Mapping for mmeId = " << mmeId << " is already known");
  m_s1apInterfaceSockets [mmeId] = Create<S1apIfaceInfo> (epcIabApplicationAddress, localS1apSocket); // TODOIAB save the Address and not the Ipv4Address, so that you can use 
...
EpcS1apEnb::DoSendInitialUeMessage (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, uint64_t stmsi, uint16_t ecgi) 
{
  ...
  Address mmeIpAddr = socketInfo->m_remoteIpAddr;
  ...
  sourceSocket->SendTo (packet, 0, mmeIpAddr);
}
...
void 
EpcS1apEnb::DoForwardIabS1apMessage(Ptr<Packet> packet, uint64_t imsi)
{
  Ptr<S1apIfaceInfo> socketInfo = m_s1apInterfaceSockets [m_mmeId]; // in case of multiple mme, extend the call
  Ptr<Socket> sourceSocket = socketInfo->m_localCtrlPlaneSocket;
  Address mmeIpAddr = socketInfo->m_remoteIpAddr;
  ...
  NS_LOG_INFO("gNB node (wired or wireless) forwards packet MME for remote imsi " << imsi);

  // store the imsi and the request on a map, so that replies will be handled correctly
  auto remoteImsiIter = m_remoteImsiRequestMap.find(imsi);
  if(remoteImsiIter == m_remoteImsiRequestMap.end())
  {
    m_remoteImsiRequestMap[imsi] = true;
  }
  else
  {
    remoteImsiIter->second = true;
  }
...
// void
// EpcS1apEnb::HandleS1ApMmeReplies()
// {
...
EpcS1apMme::AddS1apInterface (uint16_t enbId, Ipv4Address enbAddress)
{
  ...
  m_s1apInterfaceSockets [enbId] = Create<S1apIfaceInfo> (InetSocketAddress(enbAddress, m_s1apUdpPort), m_localS1APSocket); // TODO m_localS1APSocket is useless
}
                                           ...
                                           bool iab)
{
  ...
  Address enbIpAddr = socketInfo->m_remoteIpAddr;
  ...
  // Send the S1ap message through the socket
  m_localS1APSocket->SendTo (packet, 0, enbIpAddr);
}
